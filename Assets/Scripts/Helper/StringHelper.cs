...
namespace Dico.Helper
{
	public class StringHelper : MonoBehaviour
	{
		...
		public static string AddSymbolInFrontOfUpper(char symbol, string value, bool isAddToFirst = false)
		{
			string s = value;
			for (int i = isAddToFirst ? 0 : 1; i < value.Length; i++)
			{
				if(Char.IsUpper(value[i]))
				{
					s = s.Insert(i, symbol.ToString());
				}
			}
